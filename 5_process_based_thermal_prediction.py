...
OUTPUT_PREDICTIONS_FILE = os.path.join(
	MODEL_OUTPUT_DIR,
	f"final_{TARGET_YEAR}_process_based_predictions.csv",
)
...
OUTPUT_HOLDOUT_FILE = os.path.join(
	MODEL_OUTPUT_DIR,
	"process_based_holdout_last5y.csv",
)

MIN_YEAR = 1974
HOLDOUT_LAST_N_YEARS = 5
CHILL_REQ_CANDIDATES = list(range(20, 161, 5))
...
def calibrate_species_params(bloom_df, climate_df):
	...
	for species, species_rows in bloom_df.groupby("species"):
		...
		best_mae = np.inf
		for chill_req in CHILL_REQ_CANDIDATES:
			forcing_to_bloom = []
			valid_events = []
...
def main():
	...
	bloom_df = bloom_df[bloom_df["location"].isin(TARGET_PREDICTION_LOCATIONS)].copy()

	max_year = int(bloom_df["year"].max())
	holdout_start_year = max_year - HOLDOUT_LAST_N_YEARS + 1
	bloom_train_df = bloom_df[bloom_df["year"] < holdout_start_year].copy()
	bloom_holdout_df = bloom_df[bloom_df["year"] >= holdout_start_year].copy()

	if bloom_train_df.empty:
		raise ValueError("No training rows left after last-5-years holdout split.")

	print(
		f"Using years < {holdout_start_year} for calibration and years >= {holdout_start_year} as holdout."
	)

	climate_df["location"] = climate_df["location"].apply(normalize_location)
	climate_df["date"] = pd.to_datetime(climate_df["date"])
	climate_df = climate_df[climate_df["location"].isin(TARGET_PREDICTION_LOCATIONS)].copy()
	...
	print("2. Calibrating species-level chill/forcing requirements on pre-holdout years...")
	params_df = calibrate_species_params(bloom_train_df, climate_df)

	print("3. Evaluating last-5-years holdout performance...")
	...
	pred_df = predict_2026(params_df, bloom_df, forecast_df)
